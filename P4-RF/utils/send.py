...
client = InfluxDBClient(host='localhost', port=8086, username='telegraf', password='telegraf', database='ddos_realtime')
log_format = "{DATE} {IP} {MAC} {PCK} {TCP} {TCP_SYN} {UDP} {ICMP} {AVG} {IP_ENTROPY} {PORT_ENTROPY}\n"
...
def packet_summary(packet, file):
    ...
    if Extra in packet:
        ...
        # ip_port = packet[scapy.TCP].dport if scapy.TCP in packet else packet[scapy.UDP].dport
        mac_src = packet.src
        ...
        data = [
            {
                "measurement": "ddos_realtime",
                "tags": {},
                "fields": {
                    "ip_src": ip_src,
                    "mac_src": mac_src,
                    "total_packets": total_pck,
                    "tcp_packets": tcp_pck,
                    "tcp_syn_packets": tcp_syn_pck,
                    "udp_packets": udp_pck,
                    "icmp_packets": icmp_pck,
                    "avg_len": avg_len,
                    "entropy_src_ip": entropy_src_ip_val,
                    "entropy_src_port": entropy_dport_val
                }
            }
        ]

        try:
            client.switch_database('ddos_realtime')
